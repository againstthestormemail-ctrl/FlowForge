# FlowForge — codebase guide for Claude

FlowForge is a **single-file conveyor layout tool** — `FlowForge.html` runs by double-click, no build step, no external deps (parsers for PDF/DOCX are base64-embedded). Forked from OmniNapkin and rebranded.

## File layout

`FlowForge.html` (~9k lines) holds everything: HTML markup → CSS → `<script>`. The script is structured as numbered MODULE comments:
...
await p.goto('file:///home/user/FlowForge/FlowForge.html', { waitUntil: 'load' });
...
The full plan is in `FLOWFORGE_BUILD_PLAN.md`.
